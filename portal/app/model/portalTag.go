...
func (model *PortalTag) Index(db *gorm.DB, current, pageSize int, query string, queryArgs []interface{}) (paginate.Paginate, error) {
	// 获取默认的系统分页
	var total int64 = 0
	var tag []PortalTag
	...
	paginate := paginate.Paginate{Data: tag, Current: current, PageSize: pageSize, Total: total}
	if len(tag) == 0 {
