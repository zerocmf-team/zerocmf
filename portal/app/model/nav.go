...
func (model *Nav) Get(db *gorm.DB, current, pageSize int, query string, queryArgs []interface{}) (paginate.Paginate, error) {
	// 合并参数合计
	var total int64 = 0
	var nav []Nav
	...
	paginate := paginate.Paginate{Data: nav, Current: current, PageSize: pageSize, Total: total}
	if len(nav) == 0 {
...
func (model *NavItem) GetWithChildPaginate(db *gorm.DB, current, pageSize int, query string, queryArgs []interface{}) (paginate.Paginate, error) {
	...
	var total int64 = 0
	var navItem []NavItem
	tx := db.Where(query, queryArgs...).Find(&navItem).Count(&total)
	tx = db.Where(query, queryArgs...).Limit(pageSize).Offset((current - 1) * pageSize).Find(&navItem)
	if tx.Error != nil {
		return paginate.Paginate{}, tx.Error
	}
...
func (model *NavItem) OptionsList(db *gorm.DB, query string, queryArgs []interface{}) []NavItemOptions {
	...
	db.Where(query, queryArgs...).Find(&navItem)
