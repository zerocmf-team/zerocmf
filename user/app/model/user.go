...
func (u *User) AfterFind(tx *gorm.DB) (err error) {
	...
	if u.Avatar != "" {
		data, err := new(assets.Assets).GetFileUrl(u.Avatar)
		if err != nil {
			fmt.Println("err",err)
		}
		u.AvatarPrev = data.PrevPath
	}

	birthTime := time.Unix(u.Birthday, 0).Format("2006-01-02 15:04:05")
	u.BirthdayTime = birthTime

	createTime := time.Unix(u.CreateAt, 0).Format("2006-01-02 15:04:05")
	u.CreateTime = createTime

	updateTime := time.Unix(u.UpdateAt, 0).Format("2006-01-02 15:04:05")
