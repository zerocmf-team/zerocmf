...
import (
	...
	"github.com/gincmf/bootstrap/data"
...
func (model PortalPost) IndexByCategory(db *gorm.DB, current, pageSize int, query string, queryArgs []interface{}, extra map[string]string) (paginate.Paginate, error) {
	...
	tx := db.Table(prefix+"portal_post p").Select("p.*,pc.name,u.user_login").
		Joins("LEFT JOIN "+prefix+"portal_category_post cp ON p.id = cp.post_id").
		Joins("LEFT JOIN "+prefix+"portal_category pc ON pc.id = cp.category_id").
		Joins("INNER JOIN "+prefix+"user u ON u.id = p.user_id").
	...
	for k, v := range portalPostData {
		...
		portalPostData[k].PublishTime = publishTime
	...
	paginate := paginate.Paginate{Data: portalPostData, Current: current, PageSize: pageSize, Total: total}
...
func (model PortalPost) Show(db *gorm.DB, query string, queryArgs []interface{}) (PortalPost, error) {
	...
	tx := db.Table(prefix+"portal_post p").
	...
	post.ThumbPrevPath = util.FileUrl(post.Thumbnail)
	post.MoreJson.AudioPrevPath = util.FileUrl(post.MoreJson.Audio)
	post.MoreJson.VideoPrevPath = util.FileUrl(post.MoreJson.Video)
	return post, nil
...
func (model PortalPost) Update(db *gorm.DB) (PortalPost, error) {
	result := db.Save(&model)
	if result.Error != nil {
		return PortalPost{}, result.Error
	}
	return model, nil
}
