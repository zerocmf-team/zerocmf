...
type PortalPost struct {
	...
	Categories          []PortalCategories `gorm:"-" json:"Categories"`
...
type PortalCategoriesResult struct {
	Id           int    `json:"id"`
	...
	CategoriesId int    `gorm:"type:int(11);comment:分类id;not null" json:"Categories_id"`
}

// 分类关系表

type PortalCategoriesPost struct {
	Id           int     `json:"id"`
...
func (model *PortalPost) AutoMigrate(db *gorm.DB) {
	...
	db.AutoMigrate(&PortalCategoriesPost{})
...
func (model *PortalPost) ListByCategories(gormDB database.GormDB, current, pageSize int, query string, queryArgs []interface{}, extra map[string]string) (result data.Paginate, err error) {
	...
	db.Table(prefix+"portal_post p").Distinct("p.id").
		Joins("LEFT JOIN "+prefix+"portal_Categories_post cp ON p.id = cp.post_id").
		Joins("LEFT JOIN "+prefix+"portal_Categories pc ON pc.id = cp.Categories_id").
	...
	for k, v := range portalPostData {
		...
		CategoriesItem, _ := Categories.FindPostCategories(gormDB, "p.id = ? AND  p.delete_at = ?", []interface{}{v.Id, 0})
		portalPostData[k].Categories = CategoriesItem
...
func (model *PortalCategories) FindPostCategories(gormDB database.GormDB, query string, queryArgs []interface{}) ([]PortalCategories, error) {
	var Categories []PortalCategories
	prefix := gormDB.Database.Prefix
	db := gormDB.Db
	result := db.Table(prefix+"portal_post p").Select("pc.*").
		Joins("INNER JOIN "+prefix+"portal_Categories_post pcp ON pcp.post_id = p.id").
		Joins("INNER JOIN "+prefix+"portal_Categories pc ON pc.id = pcp.Categories_id").
		Where(query, queryArgs...).Scan(&Categories)
	...
	if len(Categories) == 0 {
		Categories = make([]PortalCategories, 0)
	}
